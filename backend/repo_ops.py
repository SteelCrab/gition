...
async def ensure_repo_minimal(
    user_id: int,
    user_login: str,
    repo_name: str
) -> Optional[int]:
    """
    Ensure repository exists in database with minimal info.
    Used when repository is cloned but not synced from GitHub API.
    ...
    Returns:
        Repository internal ID or None if failed
    """
    ...
    try:
        # Create minimal entry with placeholder github_repo_id
        # Use negative timestamp as placeholder to avoid conflicts
        import time
        placeholder_github_id = -int(time.time() * 1000) % 2147483647
        
        repo_id = await database.execute(
            """INSERT INTO repositories 
               (user_id, github_repo_id, name, full_name, default_branch)
               VALUES (%s, %s, %s, %s, %s)""",
            (
                user_id,
                placeholder_github_id,
                repo_name,
                f"{user_login}/{repo_name}",
                "main",
            )
        )
        
        logger.info(f"Created minimal repository entry: {user_login}/{repo_name} (id={repo_id})")
        return repo_id
    except Exception as e:
        logger.exception(f"Failed to create minimal repo entry: {e}")
        return None
